...
TEST(RootSignature, ParseRootConstants) {
  ...
  uint8_t Buffer[] = {
    ...
    0x10, 0x07, 0x93, 0x58, 0x24, 0x00, 0x00, 0x00, 0x0f, 0x00, 0x00, 0x00,
    0x0e, 0x00, 0x00, 0x00, 0x10, 0x00, 0x00, 0x00};

  EXPECT_EQ(Storage.size(), 92u);
  auto X = memcmp(Buffer, Storage.data(), 92u);
  EXPECT_TRUE(X== 0);
}
...
Parts:
  ...
  EXPECT_EQ(Storage.size(), 132u);
  EXPECT_TRUE(memcmp(Buffer, Storage.data(), 132u) == 0);
}
